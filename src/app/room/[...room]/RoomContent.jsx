...
import UpdateRoomStorage from "./UpdateRoomStorage";
...
const RoomContent = ({ data }) => {
  return (
    <div className="flex w-full flex-col justify-start items-start relative p-6 gap-2 border border-gray-200 rounded-t-lg">
      <UpdateRoomStorage data={data} />
      <div className="flex justify-between items-center w-full">
        <GoBack />
        <CopyRoomLink link={`/room/${data.id}/${data.password}`} />
      </div>
      ...
      <div className="flex justify-between items-center w-full relative">
        <div
          className="post-content text-gray-600 text-sm"
      ...
      <div className="flex justify-start items-start relative gap-6 pt-4">
        ...
        <CopyShareLinkComponent link={`/room/${data.id}/${data.password}`} />
      </div>
    </div>
  );
};
